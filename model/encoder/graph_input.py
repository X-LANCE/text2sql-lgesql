...
class RATSQLGraphInputLayer(nn.Module):
    ...
    def forward(self, batch):
        ...
        input_dict = {
            # "question": self.dropout_layer(self.question_layernorm(question + self.type_embed(batch.question_types))),
...
class RATSQLGraphInputLayerPTM(nn.Module):

    def __init__(self, ptm='bert-base-uncased', hidden_size=256, dropout=0., subword_aggregation='mean', schema_aggregation='head+tail', lazy_load=False):
        super(RATSQLGraphInputLayerPTM, self).__init__()
...
class InputRNNLayer(nn.Module):
    def __init__(self, input_size, hidden_size, cell='lstm', schema_aggregation='head+tail'):
        ...
        self.schema_lstm = self.question_lstm
        # self.schema_lstm = getattr(nn, self.cell)(self.input_size, self.hidden_size // 2, num_layers=1, bidirectional=True, batch_first=True)
    ...
    def forward(self, input_dict, batch):
        ...
        # questions = self.question_layernorm(questions + self.type_embed(batch.question_types))
        ...
        # tables = self.schema_layernorm(tables + self.type_embed(batch.table_types))
        ...
        # columns = self.schema_layernorm(columns + self.type_embed(batch.column_types))
