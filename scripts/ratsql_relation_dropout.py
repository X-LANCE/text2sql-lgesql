...
args = init_args(sys.argv[1:], task='ratsql_relation_dropout')
exp_path = hyperparam_path(args, task='ratsql_relation_dropout')
...
aug_train_dataset = Example.load_dataset('train', args.preprocess)
for ex in aug_train_dataset:
    ex.relation_dropout = True
train_dataset = train_dataset + aug_train_dataset
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        for j in range(0, nsamples, step_size):
            ...
            mask_prob = args.relation_dropout if args.relation_dropout_method == 'constant' else args.relation_dropout / 2 + float(i) * args.relation_dropout / (2 * args.max_epoch)
            current_batch = Batch.from_example_list(cur_dataset, device, train=True, method='ratsql', mask_prob=mask_prob, mask_area=args.relation_dropout_pattern)
