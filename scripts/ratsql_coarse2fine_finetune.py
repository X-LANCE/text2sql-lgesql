...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        for j in range(0, nsamples, step_size):
            ...
            current_batch = Batch.from_example_list(cur_dataset, device, train=True, method='ratsql_coarse2fine', ls=args.label_smoothing, min_rate=args.min_rate, max_rate=args.max_rate)
            loss = model(current_batch, mode='text2sql') # see utils/batch.py for batch elements
            epoch_loss += loss.item()
            loss.backward()

            # cur_dataset = [train_dataset[k] for k in train_index[j: j + step_size]]
            # current_batch = Batch.from_example_list(cur_dataset, device, train=True, method='ratsql_coarse2fine', ls=args.label_smoothing, sampling_rate=args.sampling_rate)
            # loss, prune_loss = model(current_batch, mode='multitask') # see utils/batch.py for batch elements
            # prune_loss *= args.prune_coeffi
            # epoch_prune_loss += prune_loss.item()
            # epoch_loss += loss.item()
            # total_loss = loss + prune_loss
            # total_loss.backward()
