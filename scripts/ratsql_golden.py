...
args = init_args(sys.argv[1:], task='ratsql_golden')
schema_pattern = r'_(.{4,6})_(.{4,6})_(.{4,6})_(.{4,6})$'
train_eval_schema = list(re.search(schema_pattern, args.task).groups())
train_schema, eval_schema = '-'.join(train_eval_schema[:2]), '-'.join(train_eval_schema[2:]) # schema in ['full-full', 'full-golden', 'golden-golden']

exp_path = hyperparam_path(args, task='ratsql_golden')
...
if args.read_model_path:
    model = Registrable.by_name('ratsql_golden')(params, sql_trans).to(device)
    ...
    model = Registrable.by_name('ratsql_golden')(args, sql_trans).to(device)
...
def decode(choice, output_path, acc_type='sql'):
    ...
    with torch.no_grad():
        for i in range(0, len(dataset), args.batch_size):
            current_batch = Batch.from_example_list(dataset[i: i + args.batch_size], device, train=False, method='ratsql_golden', schema=eval_schema)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        for j in range(0, nsamples, step_size):
            ...
            current_batch = Batch.from_example_list(cur_dataset, device, train=True, method='ratsql_golden', schema=train_schema)
    ...
    logger.info("Evaluation costs %.2fs ; Train/Dev dataset exact match acc is %.4f/%.4f ." % (time.time() - start_time, dev_acc, dev_acc_beam))
